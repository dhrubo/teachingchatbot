...
CREATE TABLE IF NOT EXISTS "TopicProgress" (
	...
	"confidence" varchar DEFAULT 'low' NOT NULL,
	"score" integer DEFAULT 0 NOT NULL,
	"successfulAttempts" integer DEFAULT 0 NOT NULL,
	"supportNeededAttempts" integer DEFAULT 0 NOT NULL,
	"lastPractisedAt" timestamp,
	"updatedAt" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "TopicProgress_studentId_topic_pk" PRIMARY KEY("studentId","topic")
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "TopicProgress" ADD CONSTRAINT "TopicProgress_studentId_StudentProfile_id_fk" FOREIGN KEY ("studentId") REFERENCES "public"."StudentProfile"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
