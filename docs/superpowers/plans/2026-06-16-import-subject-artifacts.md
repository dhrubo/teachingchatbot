# Import Subject Artifacts Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a database importer script to idempotently load Year 9 biology and chemistry questions from JSON files into the `mission`, `lesson`, `conceptCard`, and `questionArchetype` tables, then execute it.

**Architecture:** A standalone TypeScript script that connects to the Postgres database using Drizzle and `postgres-js`. It reads and parses JSON files, normalizes subject/domain metadata, and uses bulk upserts to populate the database tables while automatically generating high-quality GCSE-aligned Concept Cards for each lesson.

**Tech Stack:** TypeScript, Drizzle ORM, postgres (postgres-js), Node.js fs APIs.

---

### Task 1: Create Database Importer Script

**Files:**
- Create: `scripts/import-subject-artifacts.ts`

- [ ] **Step 1: Write the database importer script**

Create the importer file with the following complete code:
...
- [ ] **Step 2: Commit after writing file**

```bash
git add scripts/import-subject-artifacts.ts
git commit -m "feat: add database importer for science and maths artifacts"
```

---

### Task 2: Execute and Verify Importer Script

**Files:**
- Modify: None (pure verification)

- [ ] **Step 1: Execute the script with tsx**

Run: `npx tsx scripts/import-subject-artifacts.ts`
Expected: Success logs displaying the count of imported missions, lessons, concept cards, and question archetypes.
